...
void
utkvmmap(pagetable_t kpagetable, pagetable_t upagetable, uint64 oldsz, uint64 newsz)
{
  pte_t *kernelpte, *userpte;
  if(newsz < oldsz)
    return;

  oldsz = PGROUNDUP(oldsz);
  for(uint64 a = oldsz; a < newsz; a += PGSIZE){
    userpte = walk(upagetable, a, 0);
    kernelpte = walk(kpagetable, a, 1);
    if(userpte == 0) panic("utkvmmap: userpte should exist,but not");
    if(kernelpte == 0) panic("utkvmmap: cannot alloc kernelpte");
    // *kernelpte = *userpte & (~PTE_U);
    uint64 pa = PTE2PA(*userpte);
    *kernelpte = PA2PTE(pa) | (PTE_FLAGS(*userpte) & (~PTE_U));
  }
...
void vmprinthandler(pagetable_t cnt,int level){
  //output info of pte formally
